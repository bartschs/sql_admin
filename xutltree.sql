...
/
create sequence xxx_deptree_seq cache 200 /* cache 200 to make sequence faster */
/
drop table xxx_deptree_temptab
/
create table xxx_deptree_temptab
(
  object_id            number,
...
/
create or replace procedure xxx_deptree_fill (type char, schema char, name char) is
  obj_id number;
begin
  delete from xxx_deptree_temptab;
  commit;
  ...
  insert into xxx_deptree_temptab
    values(obj_id, 0, 0, 0);
  insert into xxx_deptree_temptab
    select object_id, referenced_object_id,
        level, xxx_deptree_seq.nextval
      from public_dependency
      connect by prior object_id = referenced_object_id
      start with referenced_object_id = xxx_deptree_fill.obj_id;
exception
  when no_data_found then
 -- raise_application_error(-20000, 'ORU-10013: ' ||
 --   type || ' ' || schema || '.' || name || ' was not found.');
    dbms_output.put_line('--------------------------------------------------------------------');
    dbms_output.put_line('ERROR: ' || type || ' ' || schema || '.' || name || ' was not found.');
    dbms_output.put_line('--------------------------------------------------------------------');
end;
/
...
set echo off
create view sys.xxx_deptree
  (nested_level, type, schema, name, seq#)
as
  select d.nest_level, o.object_type, o.owner, o.object_name, d.seq#
  from xxx_deptree_temptab d, dba_objects o
  where d.object_id = o.object_id (+)
union all
  select d.nest_level+1, 'CURSOR', '<shared>', '"'||c.kglnaobj||'"', d.seq#+.5
  from xxx_deptree_temptab d, x$kgldp k, x$kglob g, obj$ o, user$ u, x$kglob c,
      x$kglxs a
...
set echo off
create view xxx_deptree
  (nested_level, type, schema, name, seq#)
as
  select d.nest_level, o.object_type, o.owner, o.object_name, d.seq#
  from xxx_deptree_temptab d, all_objects o
  where d.object_id = o.object_id (+)
/

drop view xxx_ideptree
/
create view xxx_ideptree (dependencies)
as
