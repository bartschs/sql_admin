...
rem   dbcheck.sql
rem FUNCTION
rem   gets full snapshot of the V$ tables
rem NOTE
rem   start from specified user
rem MODIFIED
rem   16.11.03 SBartsch - made it  
rem

rem snapshot.sql - a full snapshot of the V$ tables
...
prompt
prompt -----------------------------------------------------------;
prompt DB Instance Snapshot Report
prompt -----------------------------------------------------------;
prompt
accept filename char prompt 'Spool File:    [] ' default '&TMPDIR.dbcheck.lst';
...
select '------------------------------------------------------'|| chr(10) ||
       'DB Instance Snapshot Report'|| chr(10) ||
       '------------------------------------------------------'|| chr(10) ||
...
prompt
column "miss_ratio" format 00.99
column "immediate_miss_ratio" format 00.99
